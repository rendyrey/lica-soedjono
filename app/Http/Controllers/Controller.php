...
class Controller extends BaseController
{
    ...
    public function badgeInfo()
    {
        ...
        // }
        $today = date('Y-m-d');
        $data['pre_analytics'] = \App\Transaction::where('created_time', '>=', $today)
        // ->where('is_igd', $is_igd) 
        ...
        $data['analytics'] = \App\Transaction::where('analytic_time', '>=', $today)->where('status', AnalyticController::STATUS)->count();
        ...
        $data['post_analytics'] = \App\Transaction::where('post_time', '>=', $today)->where('status', PostAnalyticController::STATUS)->count();
