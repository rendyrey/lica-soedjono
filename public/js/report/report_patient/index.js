...
var DateRangePicker = () => {
  ...
  $("#daterange-picker").on('change', function () {
      ...
      const url = baseUrl('report/patient-datatable/'+startDate+'/'+endDate+'/'+type_id);
      Datatable.refreshTableAjax(url);

      $('#print-report').attr("href", baseUrl('report/patient-print/'+startDate+'/'+endDate+'/'+type_id))
  });
...
var Select2ServerSideTest = function () {
    var _componentSelect2 = function () {
      ...
      $('.select-type').select2({
        allowClear: true,
        ajax: {
          url: baseUrl('report/select-options'),
          dataType: 'json',
          delay: 250,
          ...
          processResults: function (data, params) {
            ...
            return {
              ...
              results: $.map(data, function (item) {
                var PrefixText = '';
                if(item == 'rawat_inap'){
                    PrefixText = 'Rawat Inap';
                }else if(item == 'rawat_jalan'){
                    PrefixText = 'Rawat Jalan';
                }else{
                    PrefixText = 'IGD';
                }
