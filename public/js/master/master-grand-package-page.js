"use strict";
var masterData = 'grand-package'; // required for the url
...
var columnsDataTable = [
    ...
    { data: null, render: function(data, type, row){
            let tests = [];
            const show = 6;
            if (row.grand_package_tests.length > show) {
                row.grand_package_tests.slice(0, show).forEach(function(item, index) {
                    tests.push(`<span class="badge badge-secondary">`+item.test.id+` - `+item.test.name+` [`+item.test.initial+`]</span>`);
                });
                row.grand_package_tests.slice(show, row.grand_package_tests.length).forEach(function(item, index) {
                ...
                return tests.join(" ") + '<span class="cursor-pointer badge badge-primary package-btn-'+row.id+'" onClick="showMoreTests('+row.id+')"> show ' + (row.grand_package_tests.length - show) + ' more tests</span>';
            } else {
                row.grand_package_tests.forEach(function(item, index) {
                    tests.push(`<span class="badge badge-secondary">`+item.test.id+` - `+item.test.name+` [`+item.test.initial+`]</span>`);
                });
                return tests.join(" ");
            }
